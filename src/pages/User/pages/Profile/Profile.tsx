  ...
  return (
    <Fragment>
      ...
      <div className='mt-2 flex flex-col flex-wrap sm:flex-row'>
        <div className='truncate pt-3 capitalize sm:w-[20%] sm:text-right'>Điện thoại</div>
        <div className='sm:w-[80%] sm:pl-5'>
          <Controller
            control={control}
            name='phone'
...
type FormData = Pick<UserSchema, 'name' | 'address' | 'phone' | 'dateOfBirth' | 'avatar'>;
type FormDataError = Omit<FormData, 'dateOfBirth'> & {
  dateOfBirth?: string;
};
const profileSchema = userSchema.pick(['name', 'address', 'phone', 'dateOfBirth', 'avatar']);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const res = await updateProfileMutation.mutateAsync({
        ...
        dateOfBirth: updateData.dateOfBirth ?? new Date(1990, 0, 1),
      });
        ...
        <form className='mt-8 flex flex-col-reverse md:flex-row md:items-start' onSubmit={onSubmit}>
          <div className='mt-6 flex-grow md:mt-0 md:pr-12'>
            ...
            <Controller
              ...
              name='dateOfBirth'
              render={({ field }) => (
                <DateSelect errorMessage={errors.dateOfBirth?.message} value={field.value} onChange={field.onChange} />
              )}
            />
            <div className='mt-2 flex flex-col flex-wrap sm:flex-row'>
